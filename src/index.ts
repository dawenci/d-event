...
function bind(func: (...args: any[]) => any, thisArg: any, ...partials: any[]): (...args: any[]) => any {
  ...
  return function(...args: any[]): any {
    ...
    for (let i = 0; i < len; i += 1) {
      if (partials[i] !== bindPlaceholder) args[i] = partials[i]
    }
 ...
 * @returns
 */
function iterateEvents(iteratee: (events, name, callback, args) => any, events, name, callback, opts) {
  // 使用该正则分离用空格分割的多个事件名称
...
export default class Events {
  ...
  public on(name, callback, context) {
    ...
    if (_listening) {
      let listeners = this.__listeners__ || (this.__listeners__ = {})
  ...
  listenTo(target, name, callback?) {
    ...
    // 监听的目标的 ID
    let id = target.__listenId__ || (target.__listenId__ = uniqueId('l'))

    // 保存监听信息的对象，如果还不存在，则创建
    let listeningTo = this.__listeningTo__ || (this.__listeningTo__ = {})
  ...
  stopListening(obj, name, callback) {
    ...
    let ids = obj ? [ obj.__listenId__ ] : Object.keys(listeningTo)
  ...
  listenToOnce(obj, name, callback) {
    // Map the event into a `{event: once}` object.
    let events = iterateEvents(onceMap, {}, name, callback, bind(this.stopListening, this, obj))
